...
# Liveness guard. A milestone sits in UNDER_REVIEW only while adjudication is
# expected to succeed. If consensus cannot be reached (validators never agree,
# an evidence host stays unreachable, nobody cranks `adjudicate`), the funds
# would otherwise be frozen forever. After this many hours in UNDER_REVIEW an
# explicit, tightly authorized recovery path opens that needs no ruling.
...
class Milestone:
    ...
    submissions: u8
    round_no: u8
    review_since_hour: u64  # hour UNDER_REVIEW began; 0 when not under review
...
class VerdictEscrow(gl.Contract):
    ...
    def __init__(self):
        ...
        self.review_timeout_hours = u64(REVIEW_TIMEOUT_HOURS)
    ...
    def _return_bonds(self, ag: Agreement, m: Milestone) -> None:
        """Hand every posted bond back to whoever posted it.

        Used on recovery paths where no party lost on the merits, so nothing
        is forfeited and the escrow stays exactly solvent.
        """
        payer_bond = int(m.payer_bond)
        provider_bond = int(m.provider_bond)
        self._credit(ag.payer, u256(payer_bond))
    ...
    @gl.public.write
    def resolve_stalled_review(self, agreement_id: u32, index: u32) -> None:
        """Recovery path for a milestone frozen in UNDER_REVIEW.

        Adjudication can fail to produce a ruling at all: validators may never
        satisfy the round's agreement rule, an evidence host may stay down, or
        no one may crank `adjudicate`. Every other exit in this contract needs
        a successful ruling first, so without this method funded value would be
        locked permanently. This is the only escape that requires no verdict.

        It is bounded (`review_timeout_hours` after review began), tightly
        authorized (payer, provider or arbiter only — never a passer-by) and
        never invents an outcome in anyone's favour:

          * First-round stall with attempts left -> the milestone goes back to
            AWAITING_EVIDENCE so the provider can resubmit. Bonds returned.
          * First-round stall with attempts exhausted -> refund the payer, the
            same result as an unprovable delivery. Bonds returned.
          * Stalled appeal round (round 2+) -> DEADLOCKED, where the arbiter
            named at agreement creation settles it. Bonds stay escrowed and are
            resolved by that ruling, so appealing remains costly.
        """
        ag = self._agreement(agreement_id)
        m = self._milestone(agreement_id, index)
        sender = gl.message.sender_address
        if sender not in (ag.payer, ag.provider, ag.arbiter):
            raise gl.vm.UserError("only the payer, provider or arbiter may recover")
        if m.state != MS_UNDER_REVIEW:
            raise gl.vm.UserError("milestone is not under review")

        started = int(m.review_since_hour)
        if started == 0:
            raise gl.vm.UserError("review clock is not set")
        now = self._now_hour()
        if now <= started + int(self.review_timeout_hours):
            raise gl.vm.UserError("review timeout has not elapsed")

        if int(m.round_no) > 1:
            # A contested round stalled: hand it to the human backstop rather
            # than guessing which side the silent network would have favoured.
            m.review_since_hour = u64(0)
            m.state = MS_DEADLOCKED
            return

        self._return_bonds(ag, m)
        m.review_since_hour = u64(0)
        m.appeal_grounds = ""
        ...
        amount = int(m.amount)
        self._credit(ag.payer, u256(amount))
        ag.escrowed = u256(int(ag.escrowed) - amount)
        m.state = MS_REFUNDED
        self._maybe_close(ag)
    ...
    def get_milestone(
        ...
        return {
            ...
            "round_no": int(m.round_no),
            "review_since_hour": int(m.review_since_hour),
    ...
    def config(self) -> TreeMap[str, typing.Any]:
        return {
            ...
            "review_timeout_hours": int(self.review_timeout_hours),
