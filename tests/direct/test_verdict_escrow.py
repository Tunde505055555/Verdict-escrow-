...
# ---------------------------------------------------------------------------
# Liveness: stalled UNDER_REVIEW recovery (no ruling required)
# ---------------------------------------------------------------------------


def _submit(vm, escrow, aid, provider):
    vm.sender = provider
    escrow.submit_evidence(
        aid,
        0,
...
def test_stalled_review_returns_milestone_for_new_evidence(
    direct_vm, escrow, direct_alice, direct_bob, direct_charlie
):
    _mock_time(direct_vm)
    _mock_evidence(direct_vm)
    aid = _setup(direct_vm, escrow, direct_alice, direct_bob, direct_charlie)
    _submit(direct_vm, escrow, aid, direct_bob)
    assert escrow.get_milestone(aid, 0)["state"] == 2  # UNDER_REVIEW

    # Adjudication never lands. Too early to recover.
    direct_vm.sender = direct_alice
    with direct_vm.expect_revert("review timeout has not elapsed"):
        escrow.resolve_stalled_review(aid, 0)

    direct_vm.clear_mocks()
    _mock_time(direct_vm, NOW_HOUR + 49)  # past the 48h review timeout

    direct_vm.sender = direct_alice
    escrow.resolve_stalled_review(aid, 0)

    m = escrow.get_milestone(aid, 0)
    assert m["state"] == 1  # AWAITING_EVIDENCE
    assert m["round_no"] == 0
    assert m["evidence"] == []
    assert m["submissions"] == 1  # the stalled attempt is consumed
    assert escrow.get_agreement(aid)["escrowed"] == str(10 * GEN)


def test_stalled_review_refunds_when_attempts_exhausted(
    direct_vm, escrow, direct_alice, direct_bob, direct_charlie
):
    _mock_time(direct_vm)
    _mock_evidence(direct_vm)
    ...
    # Burn all three submission attempts on stalled reviews.
    for i in range(3):
        _submit(direct_vm, escrow, aid, direct_bob)
        direct_vm.clear_mocks()
        _mock_time(direct_vm, NOW_HOUR + 49 * (i + 1))
        _mock_evidence(direct_vm)
        direct_vm.sender = direct_bob
        escrow.resolve_stalled_review(aid, 0)

    m = escrow.get_milestone(aid, 0)
    assert m["state"] == 5  # REFUNDED
    assert escrow.credit_of(str(direct_alice)) == str(10 * GEN)
    assert escrow.get_agreement(aid)["escrowed"] == "0"
    assert escrow.get_agreement(aid)["state"] == 2  # CLOSED


def test_stalled_appeal_round_deadlocks_for_arbiter(
    direct_vm, escrow, direct_alice, direct_bob, direct_charlie
):
    _mock_time(direct_vm)
    _mock_evidence(direct_vm)
    ...
    # Round 2 never reaches consensus.
    direct_vm.clear_mocks()
    _mock_time(direct_vm, NOW_HOUR + 60)

    direct_vm.sender = direct_alice
    escrow.resolve_stalled_review(aid, 0)

    m = escrow.get_milestone(aid, 0)
    assert m["state"] == 6  # DEADLOCKED
    assert m["provider_bond"] == str(bond)  # bond stays at risk
...
def test_stranger_cannot_force_recovery(
    direct_vm, escrow, direct_alice, direct_bob, direct_charlie
):
    _mock_time(direct_vm)
    _mock_evidence(direct_vm)
    ...
    direct_vm.clear_mocks()
    _mock_time(direct_vm, NOW_HOUR + 49)

    direct_vm.sender = STRANGER
    with direct_vm.expect_revert("only the payer, provider or arbiter"):
        escrow.resolve_stalled_review(aid, 0)
